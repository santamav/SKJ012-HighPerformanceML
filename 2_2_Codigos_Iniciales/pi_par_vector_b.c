...
#pragma omp parallel default(none) \
                     shared(pi_vector, num_steps, step)
  {
    ...
    // Accumulating on auxiliar variable the computation of each thread
    double sum = 0.0; // This one should be private to each thread
    #pragma omp for
    for (int i=0; i<num_steps; i++){
      double x = (i+0.5)*step;
      sum += 4.0/(1.0+x*x);
    }
    pi_vector[rank] = sum; // Assign the accumulated value to the vector position
  }
